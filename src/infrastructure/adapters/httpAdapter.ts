import { INuweProfile, NuweProfileDTO } from "infrastructure/repositories/nuweProfile";
...
const fakeFetch  = async <T>(url: string): Promise<{status: number, data: T}> => new Promise((resolve, reject) =>
    {setTimeout( () => {
        if (url.startsWith('personal_profiles'))
            resolve({status: 200, data: PERSONAL_PROFILE as unknown as T});
        else if (url.startsWith('nuwe_profiles'))
            resolve({status: 200, data: NUWE_PROFILE as unknown as T});
...
const PERSONAL_PROFILE: PersonalProfileDTO = {
    ...
    avatarMedium: "https://i.im.ge/2021/07/22/aNpWm.png",
    avatarFull: "https://i.im.ge/2021/07/22/aNpWm.png",
    speciality: 4,
    specialityLevel: 0,
...
const ss: any[] = [{name: "Teamwork", points: 800, topPoints: 5}, {name: "Comunicación", points: 750, topPoints: 5}]; //, {name: "Lo que sea que fuere", points: 300, topPoints: 5}, {name: "Resolución problemas", points: 275, topPoints: 5}]; //{name: "Capacidad analítica", points: 4, topPoints: 5}, {name: "Capacidad analítica", points: 4, topPoints: 5}, {name: "Capacidad analítica", points: 4, topPoints: 5}],//, {name: "Capacidad analítica", points: 4, topPoints: 5}, {name: "Capacidad analítica", points: 4, topPoints: 5}, {name: "Capacidad analítica", points: 4, topPoints: 5}],
const hs: any[] = [{name: "Javascript", points: 1200, topPct: 10}, {name: "Java", points: 900, topPct: 10}, {name: "React", points: 900, topPct: 10}]; //, {name: "Angular", points: 850, topPct: 10}, {name: "CSS3", points: 800, topPct: 10}, {name: "Java", points: 1200, topPct: 10}, {name: "Java", points: 1200, topPct: 10}, {name: "Java", points: 1200, topPct: 10}, {name: "Java", points: 1200, topPct: 10}, {name: "Java", points: 1200, topPct: 10}, {name: "Java", points: 1200, topPct: 10}];
const NUWE_PROFILE: NuweProfileDTO = {
    softSkills: ss,
    hardSkills: hs,
    rankingPosition: 17,
    totalUsers: 2500,
